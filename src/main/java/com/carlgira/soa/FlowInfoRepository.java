...
public interface FlowInfoRepository extends CrudRepository<FlowInfo, Long> {
   ...
   @Query(nativeQuery = true)
    ...
    @Query(nativeQuery = true)
    List<FlowInfo> findBySensorAndDates(String sensorName, String sensorValue, Timestamp startDate, Timestamp endDate, String running, String faulted, Integer offset, Integer size);
}